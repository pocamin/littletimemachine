...
public class UpdateTaskUI extends JDialog{
	...
	private void build() {
		...
		panel.add(new JLabel(task.getTaskType().getCategory().getName()));
		...
		ok.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				try {
					...
					task = controller.save(LTMContext.getCertificat(),task);
		...
		delete.addActionListener(new ActionListener(){
			...
			public void actionPerformed(ActionEvent arg0) {
				controller.delete(LTMContext.getCertificat(),task);
				createTaskUI.dispose();
			}
